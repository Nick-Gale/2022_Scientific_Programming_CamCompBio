...
```{julia}
using Plots
pyplot()  # need pyplot for quarto notes
x = collect(0:0.1:10)
y = sin.(x)
plot(x, y; st="line", label=false, title="Sin curve",
     ylabel="sin", xlabel="x", colour=:blue)
```
...
trig_plot = plot(x, y1; title="Trigonometric Plots",
                 xlabel="x", label="sin(x)")
...
<!-- Removed own example here, as I think my attempts to show an -->
<!-- example were getting confounded by compiler. -->
...
```{julia}
x = [5, 6, 7]
[ x[i] for i in 1:3]        # okay

## [ x[i] for i in 1:4]   # bounds error
##[ @inbounds x[i] for i in 1:4] # ?? generates error in repl

function check()
    x = [5, 6, 7]
    [@inbounds x[i] for i in 1:4]
end
check()
```
